...
@section('content')
    @component('components.card')
        ...
        <div id="tabla"></div>
        @include('estimulos.factores.factor1.responsabilidades.modalEdital')
        {{-- Modal nuevo registro --}}
        @section('title_modal')
            <i class="fa fa-plus"></i> Agregar nivel de responsabilidad
        @endsection
        @section('content_modal')
            <form id="formNuevo" action="{{ route('estimulo.configuracion.factor1.responsabilidad.store') }}" method="post">
                @csrf
                @include('estimulos.factores.factor1.responsabilidades.form')
...
@section('scripts')
    ...
    <script>
        ...
        function ver_tabla(){
            $.get('tblResponsabilidades', function(data){
        ...
        function ver_datos(id){
            $.get('responsabilidades/' + id + '/edit', function(data){
                $('#id').val(data.id);
